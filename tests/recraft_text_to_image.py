...
def test_recraft_text_to_image():
    """
    Tests the Recraft AI text-to-image functionality.
    First creates a custom style using a reference image (optional), then sends a text prompt 
    to the API with the custom style and configuration parameters, and saves the resulting image 
    ...
    # Style reference URL for custom style creation
    ...
    # Parameters (using same prompt as stability_text_to_image.py for comparison)
    prompt_text = "cute cartoon cat with stripes, no background"
    negative_prompt = "blurry, low quality, distorted"
    n = 1  # Number of images to generate (1-6)
    base_style = "digital_illustration"  # Base style for custom style creation
    model = "recraftv3"  # Options: recraftv3, recraftv2
    response_format = "url"  # Options: url, b64_json
    size = "1024x1024"  # Size in WxH format
    ...
    use_custom_style = True  # Set to False to use base_style instead of custom style
    
    # Background removal option
    remove_background = True  # Set to True to remove background after generation
    
    output_dir = "tests/outputs"
    output_filename = "recraft_text_to_image_output.png"
    output_path = os.path.join(output_dir, output_filename)
    
    # Background removal output path
    bg_removed_filename = "recraft_text_to_image_no_bg.png"
    bg_removed_path = os.path.join(output_dir, bg_removed_filename)

    os.makedirs(output_dir, exist_ok=True)

    # 1. Get or create custom style (if enabled)
    ...
    # 2. API Request
    step_start_time = time.time()
    print(f"Sending request to Recraft AI text-to-image endpoint: {api_endpoint}...")
    if use_custom_style and custom_style_id:
        print(f"Using custom style ID: {custom_style_id}")
    else:
        print(f"Using base style: {base_style}")
    ...
    # Use custom style if available, otherwise use base style
    if use_custom_style and custom_style_id:
        data["style_id"] = custom_style_id
    else:
    ...
    print(f"\n--- Configuration Used ---")
    print(f"Endpoint: Recraft Text-to-Image")
    if use_custom_style:
        if create_new_style:
        ...
        print(f"Base Style: {base_style} (for custom style creation)")
    else:
        print(f"Style: {base_style} (base style, no custom style)")
    ...
    if use_custom_style and create_new_style:
        print(f"Style Creation Cost: 40 API Units ($0.04)")
        total_cost_units += 40
        total_cost_usd += 0.04
    else:
        print(f"Style Creation Cost: 0 API Units ($0.00) - Used existing style or base style")
...
if __name__ == "__main__":
    print("Running Recraft AI text-to-image test...")
    try:
        test_recraft_text_to_image()
