...
@pytest.mark.asyncio
async def test_generate_image_to_image_nano_banana(request):
    """Test 1.5: /generate/image-to-image endpoint (Google Nano Banana via Replicate)."""
    start_time = time.time()
    client_task_id = f"test-i2i-nano-banana-{uuid.uuid4()}"
    ...
    endpoint = f"{BASE_URL}/generate/image-to-image"
    image_to_upload_url = "https://iadsbhyztbokarclnzzk.supabase.co/storage/v1/object/public/makeit3d-public//portrait-boy.jpg"
    prompt = "Make him look like a circus clown."
    ...
    # Call Google Nano Banana endpoint
    request_data = {
        "task_id": client_task_id,
        "provider": "google",
        "input_image_asset_url": input_supabase_url,
        "prompt": prompt,
        "google_model": "nano-banana",
    ...
    # Wait for Celery task completion (Google Nano Banana is asynchronous via Replicate)
    polling_start = time.time()
    task_result_data = await wait_for_celery_task(task_id, "Google Nano Banana", poll_interval=5, total_timeout=300.0)
    ai_processing_time = time.time() - polling_start
    
    print(f"🤖 Google Nano Banana Processing completed in {ai_processing_time:.2f}s")

    asset_url = task_result_data['asset_url']
    concept_file_path, concept_download_time = await download_file(asset_url, request.node.name, "nano_banana_concept.jpg")
    ...
    timings = {
        ...
        "Input Upload": upload_time,
        "API Response Time": api_response_time,
        "Google Nano Banana Processing": ai_processing_time,
        "Concept Download": concept_download_time
    }
    locations = {
        "supabase_storage": {"input_image_asset_url": input_supabase_url, "concept_asset_url": asset_url},
        "local_files": {"nano_banana_concept.jpg": concept_file_path}
    }
    print_test_summary(request.node.name, client_task_id, start_time, timings, locations) 