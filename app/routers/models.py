...
from app.sync_state import sync_task_results # Import from the new sync_state module
from app.config import settings # Import settings
from app.supabase_client import download_image_from_storage # Import the missing function
...
# Removed sync_task_results from here - it's now in app/sync_state.py
# In-memory store for synchronous task results (for testing/synchronous mode)
...
async def get_task_status(
    ...
    # --- Check in-memory store for synchronous task results ONLY if sync_mode is enabled ---
    if settings.sync_mode:
        logger.info("Sync mode is enabled. Checking synchronous results store.")
        if task_id in sync_task_results:
            logger.info(f"Task {task_id} found in synchronous results store. Returning completed status.")
            result_data = sync_task_results[task_id]
            # Assuming the stored result data is already in the format expected by TaskStatusResponse.result
            # The poll_task_status expects a dictionary with 'image_urls' for OpenAI.
            ...
            return TaskStatusResponse(
                status='completed',
                progress=100.0,
                result_url=first_image_url, # Set result_url to the first image URL for compatibility
                result=result_data # Return the full stored result data
            )
        else:
            # If in sync mode but task_id not found (shouldn't happen with current flow, but for safety)
            logger.warning(f"Task {task_id} not found in synchronous results store in sync mode.")
            raise HTTPException(status_code=404, detail=f"Task {task_id} not found in synchronous mode.")

    else:
        # --- If sync_mode is NOT enabled, get task from Celery backend ---
        logger.info("Sync mode is NOT enabled. Checking Celery backend.")
